...
{   
  ...
  if(_readStringUntil() > 0) 
    {       
      ...
      if(manual_on)
       {      
              Serial.println(first_data);  Serial3.println(first_data);
...
//---------------- 
   ...
   { 
      ...
      digitalWrite(air_motor, HIGH); 
      digitalWrite(incoming_water, LOW);      
      digitalWrite(outgoing_water, HIGH);    Serial.println("outgoing_water on");
   }
   else if((water_change==1) && (Distance > Distance_Max))
   { 
      water_change=2;
      digitalWrite(outgoing_water, LOW);     Serial.println("outgoing_water off");
      digitalWrite(incoming_water, HIGH);    Serial.println("incoming_water on");
   }
...
{        
     ...
     if (Serial.available())       //seri port için
      {  
      ...
      if (Serial3.available())     // BLUETOOTH için
      {  
...
void Smart_App_Cnt()
  { 
       Serial.println(last_data);  
       String tempa="";  byte i=0, j=2;

       tempa=last_data.substring(i,i+j); i=i+2; //Serial.print(tempa);
    if(tempa=="B1")       
         digitalWrite(Buzzer, HIGH);
    else if(tempa=="B0")
         digitalWrite(Buzzer, LOW);       
 ...
 if(((tempa=="U1")||(tempa=="U0")) && (!digitalRead(incoming_water)))  
  {
    ...
    if(tempa=="D1")       
       ...
       tempa=last_data.substring(i,i+j); i=i+2; //Serial.println(tempa);
    if(tempa=="W1")       
         digitalWrite(water_valve, HIGH);
    else if(tempa=="W0")
          digitalWrite(water_valve, LOW);
...
void Bluetooth_Setting()
{
     ...
     // String uart = "9600,0,0";
      ...
      Serial3.print("AT+NAME=");//"AT+NAME=Bluet_Cnt_Car_rk"
