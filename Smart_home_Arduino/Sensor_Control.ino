...
{     //alarm_condition=0;
       ...
       if((digitalRead(Rain_sens)) && (!digitalRead(rain_cnt_device))) 
        {
           Serial.println("Rain_Sens_on");  
           digitalWrite(rain_cnt_device, HIGH);
        }
       else if((!digitalRead(Rain_sens)) && (digitalRead(rain_cnt_device)))
        {
           Serial.println("Rain_Sens_of");   
           digitalWrite(rain_cnt_device, LOW);
        }    

        if((digitalRead(soil_moisture_sens)) && (!digitalRead(soil_moisture)))   
        {
           Serial.println("soil_moisture_sens_on");   
           digitalWrite(soil_moisture, HIGH);
        }
        else if((!digitalRead(soil_moisture_sens)) && (digitalRead(soil_moisture)))
        {
           Serial.println("soil_moisture_sens_of");   
           digitalWrite(soil_moisture, LOW);
        } 
       ...
       {
          bitWrite(alarm_condition,1,1);        Gas_Smoke_alarm();  
       }
       else
       ...
       {
          bitWrite(alarm_condition,2,1);        Gas_Smoke_alarm(); 
       }
       else
   ...
   if((!digitalRead(Gas_Sensor))&&(!digitalRead(Smoke_Sensor))&&(!digitalRead(Fire_Sensor)))
     {
       ...
       {  // air_motor NK!!!!!
          digitalWrite(outgoing_water, LOW);    digitalWrite(incoming_water, LOW); 
          digitalWrite(air_motor, HIGH);        Serial.println("air_motor off");
          Serial.println("low water level");    Serial3.println("low water level");
       }      
...
void Gas_Smoke_alarm()
  { 
     digitalWrite(air_motor, LOW);         digitalWrite(water_valve, LOW);
     digitalWrite(electricity, HIGH);      digitalWrite(gas, HIGH); 
     digitalWrite(incoming_water, LOW);    digitalWrite(outgoing_water, LOW);
  }
//-------------------------------------------------------------------------- 
