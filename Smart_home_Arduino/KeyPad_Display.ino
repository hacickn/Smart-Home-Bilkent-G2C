...
void Display_Refresh()
  {       
        lcd.setCursor(0,  0);     lcd.print(Distance);              // 1.satır için 
        lcd.setCursor(3,  0);     lcd.print("cm,");        
        lcd.setCursor(7,  0);     lcd.print(temp);
        lcd.setCursor(11, 0);     lcd.print("'C,");  
        lcd.setCursor(15, 0);     lcd.print(humidity);
    ...
    for (int i=0; i < 4; i++) // 
        {  lcd.setCursor(14+i, 1);  lcd.print(Data_Psw[i]);  }                  
             
    if(alarm_condition > 0)                                         // 3.satır için 
     {
        lcd.setCursor(0, 2);    lcd.print("                     "); 
        lcd.setCursor(0, 2);    lcd.print(alarm_message);    
     ...
     { 
        ...
        lcd.setCursor(0,  3);    lcd.print(_hour);                   // 4.satır için 
        lcd.setCursor(2,  3);    lcd.print(':');
        lcd.setCursor(3,  3);    lcd.print(_minute);
        lcd.setCursor(5,  3);    lcd.print(':');
        lcd.setCursor(6,  3);    lcd.print(_second);
        
        //lcd.setCursor(8,  3);    lcd.print(_day_week_tr);
        
        lcd.setCursor(10, 3);   lcd.print(_day);
        lcd.setCursor(12, 3);   lcd.print('.');
        lcd.setCursor(13, 3);   lcd.print(_moon);
        lcd.setCursor(15, 3);   lcd.print('.'); 
        lcd.setCursor(16, 3);   lcd.print(_year);   
  }
//--------------------------------------------------------------------------
void Key_Pad()
{
      ...
      {             
       ...
       { 
           ...
           if((door_psw_enb) && (!door_encrypted))// zaman dolmadıysa
            {
              give_change=3;    psw_time=1;    
            }
           else if((!door_psw_enb) && (door_encrypted))
            {
              door_encrypted=false;            door_psw_enb=false;  
              give_change=3;                   psw_time = 0; 
              digitalWrite(door, HIGH);        door_time= 1;
              bitWrite(alarm_condition,3,0);   bitWrite(alarm_condition,4,0);             
            }          
        }
        else         
        {
...
{   
     ...
     Psw_count =0;    Data_Psw="";  
}
//--------------------------------------------------------------------------
void changePassword()
{
     lcd.clear();
     lcd.setCursor(0, 0);   lcd.print("Master psw:");  
     
     Get_new_psw(1);                                 //master şifre dogru ise
     String a= Data_Psw; 
     String b= Master_Psw;
     
  if(a!=b)
    { Serial.print("Fault_KeyPad Master ");
      lcd.setCursor(0, 2);   lcd.print("Fault_KeyPad Master"); 
      return;
    }     

     lcd.clear();
     lcd.setCursor(0, 0);   lcd.print("Type new psw:"); 
  ...
  if(a==b)
    {       
       ...
       Master_Psw=Data_Psw;    Serial.println(Master_Psw);
...
{  
  ...
  { 
    ...
    {
      Data_Psw = Data_Psw  + customKey;
      lcd.setCursor(Psw_count, line); lcd.print(Data_Psw[Psw_count]);
